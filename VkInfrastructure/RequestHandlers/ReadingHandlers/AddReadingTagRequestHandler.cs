...
{
    public class AddReadingTagRequestHandler : IRequestHandler<AddReadingTagRequest, DtoResult<IEnumerable<TagDto>>>
    {
        private readonly VkDbContext _context;
        private readonly DtoResult<IEnumerable<TagDto>> _response;

        public AddReadingTagRequestHandler(VkDbContext context)
        {
            _context = context;
            _response = new DtoResult<IEnumerable<TagDto>>();
        }

        public async Task<DtoResult<IEnumerable<TagDto>>> Handle(AddReadingTagRequest request, CancellationToken cancellationToken)
        {
