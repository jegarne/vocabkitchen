...
export class TextSelectDirective implements OnInit, OnDestroy {
  ...
  public ngOnDestroy(): void {
    ...
    this.elementRef.nativeElement.removeEventListener("touchstart", this.handleMousedown, false);
  ...
  public ngOnInit(): void {
    ...
    this.zone.runOutsideAngular(
      () => {
        ...
        this.elementRef.nativeElement.addEventListener("touchstart", this.handleMousedown, false);

        // While the mouse-event takes care of starting new selections within the
  ...
  private getContentItemRange(range: Range, text: any): ContentItemRange {

    let leftTrimLength = text.length - text.trimStart().length;

    let result = {
      contentItemStartIndex: 0,
      contentItemEndIndex: 0,
      start: range.startOffset + leftTrimLength,
      end: range.startOffset + leftTrimLength + text.trim().length -1
    };

    if (range.endContainer === range.startContainer) {
      // selection is in the same text node
      let container = range.commonAncestorContainer.parentNode.parentNode;
      let elementTypeNodes = [].filter.call(container.childNodes, function (el) { return el.nodeType === 1 });
      result.contentItemStartIndex = Array.prototype.indexOf.call(elementTypeNodes, range.startContainer.parentNode);
      result.contentItemEndIndex = Array.prototype.indexOf.call(elementTypeNodes, range.endContainer.parentNode);

    } else {
      // selection is across spans
  ...
  private processSelection(): void {
    ...
    if (this.elementRef.nativeElement.contains(rangeContainer)) {
      ...
      var contentItemRange = this.getContentItemRange(range, selection.toString());
      ...
      this.zone.runGuarded(
        () => {
          ...
          this.textSelectEvent.emit({
            ...
            contentItemRange: contentItemRange,
            viewportRectangle: {
